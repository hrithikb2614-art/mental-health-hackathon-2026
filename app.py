...
tab_kids, tab_checker, tab_assistant = st.tabs(
    ["🧒 Kids & Teens Check-In", "🔍 Symptom Checker", "🤖 AI Assistant"]
)
...
with tab_assistant:
    ...
    st.caption(
        ...
        "this app's knowledge base below. This is not a diagnosis and does not "
        "replace a real conversation with a qualified professional."
    )
    ...
    if not rag.is_configured():
        ...
        with st.expander("📄 Load a document into the knowledge base (admin only)"):
